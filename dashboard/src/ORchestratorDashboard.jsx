...
const SIGMA_MAP   = { Low:0.10, Standard:0.15, High:0.25 };
const OVERRUN_MAP = { Low:"4%", Standard:"8%", High:"15%" };

const timeToMin = (t) => { const [h,m] = t.split(":").map(Number); return h*60+m; };
const minToTime = (m) => `${String(Math.floor(m/60)%24).padStart(2,"0")}:${String(Math.round(m)%60).padStart(2,"0")}`;

const OR_START = timeToMin("07:30");
const OR_END   = timeToMin("15:30");
const DAY_W    = OR_END - OR_START + 30;
const pxPos    = (min, width) => ((min - OR_START) / DAY_W) * width;

// ── Mock fallback ─────────────────────────────────────────────────────────────
const buildMockSchedule = (gamma) => {
  let t = OR_START;
  let total = 0;
  const schedule = [];
  const rejected = [];
  for (const p of FHIR_PATIENTS) {
    const sigma = SIGMA_MAP[p.risk] ?? 0.15;
    const buf = Math.round(gamma * sigma * Math.sqrt(p.duration_p50) * 10) / 10;
    const scheduled = Math.round((p.duration_p50 + buf) * 10) / 10;
    if (total + scheduled > 480) {
      rejected.push({ ...p, buffer_minutes:buf, scheduled_minutes:scheduled,
        status:"EXCLUDED", reason:"OR capacity exceeded" });
      continue;
    }
    const start = minToTime(t);
    t += scheduled;
    total += scheduled;
    schedule.push({ ...p, buffer_minutes:buf, scheduled_minutes:scheduled,
      start, end:minToTime(t),
      overrun_probability:OVERRUN_MAP[p.risk] ?? "8%",
      order:schedule.length+1 });
  }
  const util = Math.round(total/480*100*10)/10;
  return { schedule, rejected,
    summary:{ total_minutes:Math.round(total*10)/10, utilization_percent:util,
      overrun_risk:util>95?"High":util>85?"Medium":"Low", gamma } };
};

// ── Components ────────────────────────────────────────────────────────────────
const KPICard = ({ value, label, color, sub }) => (
...
const TimeAxis = ({ width }) => {
  const hours = [];
  for (let m = OR_START; m <= OR_END; m += 60) hours.push(m);
  return (
    <div style={{ position:"relative", height:20, marginBottom:4 }}>
      {hours.map(m => (
        <span key={m} style={{ position:"absolute", left:pxPos(m,width),
...
const GanttBar = ({ op, width }) => {
  ...
  const L = pxPos(startM, width);
  const W = Math.max(((endM-startM)/DAY_W)*width, 6);
  const bufFrac = op.scheduled_minutes > 0 ? op.buffer_minutes/op.scheduled_minutes : 0;
  return (
    <div style={{ position:"relative", height:36, marginBottom:4 }}>
      ...
      <div title={`${op.patient_id} · ${op.procedure} · ${op.start}–${op.end} · +${op.buffer_minutes}'`}
...
const GanttBarRejected = ({ op, width }) => (
  <div style={{ position:"relative", height:36, marginBottom:4 }}>
    <div style={{ position:"absolute", left:0, top:4, height:28, width:width,
      background:"#ffffff06", border:"1px dashed #2a2a38", borderRadius:5,
      display:"flex", alignItems:"center", paddingLeft:10, gap:8 }}>
      <span style={{ fontSize:9, fontFamily:"'JetBrains Mono',monospace", color:"#333" }}>❌</span>
      <span style={{ fontSize:9, fontFamily:"'JetBrains Mono',monospace", color:"#333" }}>
        {op.procedure} · brak miejsca w OR
      </span>
    </div>
  </div>
);
...
export default function ORchestratorDashboard() {
  ...
  const applyData = useCallback((data, live) => {
    const withNames = data.schedule.map(op => ({
      ...op, name: FHIR_PATIENTS.find(p => p.patient_id === op.patient_id)?.name ?? op.patient_id,
    }));
    const rejWithNames = (data.rejected ?? []).map(op => ({
      ...op, name: FHIR_PATIENTS.find(p => p.patient_id === op.patient_id)?.name ?? op.patient_id,
    }));
    setSchedule(withNames);
    setRejected(rejWithNames);
  ...
  const fetchSchedule = useCallback(async (g) => {
    ...
    try {
      const res = await fetch(MCP_URL, {
        ...
        body:JSON.stringify({ patients:FHIR_PATIENTS, gamma:g, or_capacity_minutes:480, start_time:"07:30" }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      applyData(await res.json(), true);
    } catch(e) {
      applyData(buildMockSchedule(g), false);
      setError(`MCP niedostępny (${e.message}) — tryb demo`);
    } finally {
      setLoading(false);
    }
  }, [applyData]);

  useEffect(() => { fetchSchedule(0.5); }, []);
    ...
    <div style={{ minHeight:"100vh", background:"#0a0a0f", color:"#ddd",
      ...
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:10, letterSpacing:"0.22em", color:"#333",
          ...
          ORchestrator · MIT Robust Scheduling · FHIR R4
        </div>
        <div style={{ display:"flex", alignItems:"baseline", gap:16, flexWrap:"wrap" }}>
          ...
          <span style={{ fontSize:11, color:"#444", fontFamily:"'JetBrains Mono',monospace" }}>
            {FHIR_PATIENTS.length} pacjentów FHIR
          </span>
          <span style={{ fontSize:10, fontFamily:"'JetBrains Mono',monospace",
            color:mcpLive?"#6bcb77":"#ff9f43",
            background:mcpLive?"#6bcb7718":"#ff9f4318",
            border:`1px solid ${mcpLive?"#6bcb7744":"#ff9f4344"}`,
            borderRadius:4, padding:"2px 8px" }}>
            {mcpLive ? "● MCP live" : "○ demo mode"}
          </span>
      ...
      {/* Error */}
      {error && (
        <div style={{ background:"#ff9f4312", border:"1px solid #ff9f4333",
          borderRadius:8, padding:"8px 16px", marginBottom:14,
          fontSize:10, color:"#ff9f43", fontFamily:"'JetBrains Mono',monospace",
          display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <span>ℹ {error}</span>
          <button onClick={() => fetchSchedule(gamma)} style={{ background:"transparent",
            border:"1px solid #ff9f43", color:"#ff9f43", borderRadius:4,
            padding:"2px 10px", cursor:"pointer", fontSize:10 }}>Retry MCP</button>
        </div>
      )}

      {/* KPI */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:10, marginBottom:18 }}>
        <KPICard value={summary?`${summary.utilization_percent}%`:"—"} label="Wykorzystanie"
          color={summary?.utilization_percent>=80?"#6bcb77":"#e0c039"} sub="480 min capacity" />
        <KPICard value={schedule.length?schedule.at(-1).end:"—"} label="Koniec dnia"
          color={overtime?"#ff6b6b":"#6bcb77"} sub={overtime?"⚠ nadgodziny":"✓ na czas"} />
        <KPICard value={`${schedule.length}/${FHIR_PATIENTS.length}`} label="Zaplanowane"
          color="#e0c039" sub="op. w harmonogramie" />
      ...
      {/* Γ Slider */}
      <div style={{ background:"#111118", border:"1px solid #00d4ff33", borderRadius:10,
        ...
        <div>
          ...
          <div style={{ fontSize:10, color:"#444", fontFamily:"'JetBrains Mono',monospace" }}>
            buffer = Γ × σ × √P50 · per patient risk level
          </div>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:12 }}>
        <div style={{ background:"#111118", border:"1px solid #1e1e2a",
          ...
          <div style={{ fontSize:10, letterSpacing:"0.1em", color:"#444",
            ...
            Harmonogram · Gantt · Γ={gamma.toFixed(1)}
            {mcpLive && <span style={{ color:"#6bcb77", marginLeft:8 }}>· MCP live</span>}
          </div>
          {schedule.length === 0 && !loading ? (
            <div style={{ color:"#333", fontFamily:"'JetBrains Mono',monospace",
              fontSize:11, padding:"20px 0" }}>Kliknij ▶ Przelicz</div>
          ) : (
            <div style={{ overflowX:"auto" }}>
              <div style={{ minWidth:700 }}>
                ...
                <div style={{ position:"relative" }}>
                  <div style={{ position:"absolute", left:130+pxPos(OR_END,GANTT_W),
                  ...
                  {rejected.length > 0 && (
                    <div style={{ marginTop:8, borderTop:"1px dashed #1e1e2a", paddingTop:8 }}>
                      <div style={{ fontSize:9, color:"#333", letterSpacing:"0.1em",
                        textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace",
                        marginBottom:6, paddingLeft:4 }}>
                        Odrzuceni — brak miejsca ({rejected.length})
                      </div>
                      {rejected.map(op => (
                        <div key={op.patient_id}
                          style={{ display:"grid", gridTemplateColumns:"130px 1fr",
                            alignItems:"center", borderRadius:6, padding:"0 4px", opacity:0.5 }}>
                          <div style={{ paddingRight:8, paddingLeft:2 }}>
                            <div style={{ fontSize:11, fontWeight:600, color:"#444" }}>{op.name}</div>
                            <div style={{ fontSize:9, fontFamily:"'JetBrains Mono',monospace",
                              color:"#333" }}>{op.patient_id} · {op.risk}</div>
                          </div>
                          <GanttBarRejected op={op} width={GANTT_W} />
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div style={{ display:"flex", gap:14, marginTop:10, flexWrap:"wrap" }}>
            {[["#00d4ff","Γ buffer"],["#ff2244","HIGH"],["#ff9f43","MED"],
              ["#6bcb77","LOW"],["#2a2a38","❌ odrzucony"]].map(([c,l]) => (
        ...
        {/* Patient list */}
        <div style={{ background:"#111118", border:"1px solid #1e1e2a",
          borderRadius:10, padding:"18px 16px", overflowY:"auto", maxHeight:600 }}>
          <div style={{ fontSize:10, letterSpacing:"0.1em", color:"#444",
            textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:12 }}>
            Pacjenci · FHIR R4
          </div>
            ...
            return (
              <div key={op.patient_id} className="row-hover"
                ...
                style={{ padding:"10px", borderRadius:7, marginBottom:5,
                  ...
                  background:isSelected?`${color}0d`:"transparent", transition:"all 0.15s" }}>
          ...
          {rejected.length > 0 && (
            <div style={{ marginTop:8, borderTop:"1px solid #1e1e2a", paddingTop:8 }}>
              <div style={{ fontSize:9, color:"#333", letterSpacing:"0.1em",
                textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace",
                marginBottom:6 }}>Odrzuceni ({rejected.length})</div>
              {rejected.map(op => (
                <div key={op.patient_id} style={{ padding:"8px 10px", borderRadius:7,
                  marginBottom:5, border:"1px dashed #1e1e2a", opacity:0.5 }}>
                  <div style={{ fontSize:11, fontWeight:600, color:"#444",
                    marginBottom:2 }}>{op.name}</div>
                  <div style={{ fontSize:10, color:"#444",
                    fontFamily:"'JetBrains Mono',monospace" }}>{op.procedure}</div>
                  <div style={{ fontSize:9, color:"#ff9f4388",
      ...
      {selected && (
        <div className="fade-in" style={{ marginTop:12, background:"#111118",
          ...
          {[
            ...
            { label:"Overrun risk", value:selected.overrun_probability, mono:true,
            ...
            <div key={f.label}>
              ...
              <div style={{ fontSize:f.mono?14:13, fontWeight:600, color:f.color??"#ccc",
      ...
      {/* Summary */}
      {summary && (
        <div style={{ marginTop:12, background:"#111118", border:"1px solid #1e1e2a",
          borderRadius:10, padding:"14px 20px",
          display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16 }}>
          {[
            { label:"Źródło",       value:mcpLive?"MCP live ✓":"Demo mode", color:mcpLive?"#6bcb77":"#ff9f43" },
            { label:"Utilization",  value:`${summary.utilization_percent}%`, color:"#6bcb77" },
            { label:"Overrun Risk", value:summary.overrun_risk,
              color:summary.overrun_risk==="Low"?"#6bcb77":summary.overrun_risk==="Medium"?"#ff9f43":"#ff2244" },
            { label:"Gamma",        value:`Γ = ${summary.gamma}`, color:"#00d4ff" },
          ].map(f => (
            <div key={f.label}>
              <div style={{ fontSize:9, color:"#444", textTransform:"uppercase",
              ...
              <div style={{ fontSize:16, fontWeight:700, color:f.color,
                fontFamily:"'JetBrains Mono',monospace" }}>{f.value}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ marginTop:24, paddingTop:12, borderTop:"1px solid #1e1e2a",
        ...
        <span>FHIR R4 · MIT Robust Scheduling · Denton et al.</span>
      </div>
    </div>
  );
}
